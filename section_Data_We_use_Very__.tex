...
\tablecaption{Parameters of Interferometric Maps\label{table:obs}}
\tablewidth{0pt}
\tablehead{\colhead{Source} & \colhead{RA} & \colhead{Dec} & \colhead{beamsize} & \colhead{rms} & \colhead{Outflow PA\tablenotemark{a}}\\ 
\colhead{} & \colhead{(hh:mm:ss)} & \colhead{(dd:mm:ss)} & \colhead{} & \colhead{(mJy beam$^{-1}$)} & \colhead{(deg)} }
\tablenotetext{a}{Measured East from North}
